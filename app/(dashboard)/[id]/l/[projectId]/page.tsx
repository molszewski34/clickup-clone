...
export default async function Page({ params }: { params: Promise<{ projectId: string }> }) {
  await params;
  const tasksInProgress = MOCK_TASKS.filter((task) => task.status === TaskStatus.inProgress);
  const tasksTodo = MOCK_TASKS.filter((task) => task.status === TaskStatus.todo);
  const tasksCompleted = MOCK_TASKS.filter((task) => task.status === TaskStatus.completed);
